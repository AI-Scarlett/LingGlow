...
12. Give every Codex theme a dedicated 3:1 home artwork that is visually related to, but not the same file as, the global background. Author it through the existing `advanced.banner` contract: `top-right`/`bottom-right` reserve the left side and `top-center` reserves the center. The runtime derives its bounded focus and directional edge-gradient scrim from that declared position and banner opacity; do not add unsupported union-profile fields. Verify the result separately under the light and dark shells, and never darken the whole frame or drop a page-sized veil to rescue a busy image. See `references/codex-hero-composition.md`.
...
26. On Codex, a theme's layout may select only a runtime-implemented `taskMode` of `ambient`, `banner`, or `off`. Do not introduce any layout that reorders, regrids, or rebuilds the native home DOM, and do not use positional child selectors (`div:first-child`, `div:nth-child(n)`) or layout classes to reshuffle native blocks. A richer layout may open only after it has a dedicated exact adapter and an isolated real-client regression matrix; until then Codex stays on the generic-safe surface set with its native home geometry preserved. This guardrail is Codex-scoped; it does not add a layout concept to WorkBuddy or Doubao.
27. Never generate or accept key art that contains fabricated UI chrome: buttons, toolbars, window frames, title bars, scrollbars, tabs, or a fake cursor. Each client renders its own real chrome, so painted chrome stacks a second frame over it. This holds across all three clients — the Codex 3:1 home artwork, the WorkBuddy 16:9 project hero, and the Doubao global background alike — and reinforces the transparent-subject rules in contracts 7 and 8.
28. Enforce contract 13's third-party-IP exclusion as a generation-time hard gate, not only an intake filter, because the upstream failure that shipped recognizable fan art happened at the prompt stage. The image-generation prompt must never drift into a recognizable third-party game, anime, or film character; a faction, world, or place proper noun from such a work; a brand logo, wordmark, or mascot; or a reproduction of another product's software interface (its trade dress). These stay excluded even when a user asks for them and even when an upstream preset already ships the art: a repository license conveys only the rights the licensor actually holds, and AI regeneration of an existing character does not launder it. A real person or a named brand offered as a deliberate fan or Agent theme is the one gated exception and must run through contract 24 with source, license, redistribution terms, and unofficial-fan-made status recorded first. Treat `同人`, `游戏`, `角色`, `fan`, or any named IP appearing in the tags or brief as risk self-evidence that forces human review; when rights are unclear, reject rather than quietly substitute a vague abstraction.
...
3. Fill one client input template. Separate global background, home artwork, content surfaces, overlay surfaces, text tokens, selection states, semantic action controls, and optional branding. For Codex, record the `advanced.banner.position`/`opacity` values and the authoring-only derived composition hints in `artwork.homeHero`; persist only the existing banner fields, not the hints. Complete the required `provenance` block and follow `references/codex-hero-composition.md`.
4. Approve the full-resolution master before deriving client assets. Reject generic abstraction, weak brand recognition, pasted floating logos, an Agent mark with no visible computational function, unrequested people in logo-led skins, visibly synthetic anatomy, a focal subject that collides with the left UI-safe region, a focal subject that falls inside the declared `safeArea` instead of opposite it, any fabricated UI chrome baked into the art (contract 27), or any unrequested recognizable third-party character, faction name, brand mark, or software trade dress (contract 28).
...
- Author the home artwork side through the existing `advanced.banner.position` field. The runtime deterministically maps right-positioned banners to `safeArea=left, focusX=.72` and centered banners to `safeArea=center, focusX=.5`; image saliency remains QA metadata and must not override the author signal. The right-panel fallback, directional scrim, and derived `ambient`/`banner`/`off` mode must stay consistent with that mapping. Do not add new union-schema fields for these derived values.
