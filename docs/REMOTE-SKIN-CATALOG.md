...
灵妆 2.2.1 起使用目录协议 v2，把皮肤发现、分类搜索、下载和运行时应用拆成独立阶段。App 包只保留运行时、兼容小资源、一套免费皮肤和两套 VIP 离线模板；其余皮肤由 GitHub 目录按需分发。
...
2. 目录卡片只读取名称、说明、明暗模式、Agent、分类、系列、标签、预览 URL、包大小和 SHA-256。
...
- `category`：稳定主分类，当前支持 `sports`、`fantasy`、`nature`、`minimal`、`art`、`seasonal`、`other`。
- `series`：同系列模板共用的稳定小写 ID。
- `tags`：用于名称之外的本地搜索，描述题材、氛围、明暗和用途。

客户端会比较 `catalogVersion`，发现模板目录更新时显示提示；用户也可以手动点击“拉取新模板”。目录更新与 App 自动升级相互独立。
