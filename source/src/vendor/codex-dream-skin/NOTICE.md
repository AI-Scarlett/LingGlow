...
`dream-skin.css` in this directory is **not imported or read at runtime** and is retained for reference only. The live injected CSS is generated from the `cssText` in `src/codex-dream-skin-adapter.mjs`; the vendor renderer template loaded by the adapter is `renderer-inject.js`.
